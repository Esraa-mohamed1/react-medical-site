...
const PaymentPage = ({ onPaymentSuccess }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/api/medical/appointments/${appointment_id}/`, {
  ...
  const createOrder = async (data, actions) => {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/users/paypal/create-order/?appointment_id=${appointment_id}`,
        {},
        {
          headers: {
              ...
              onApprove={async (data, actions) => {
                ...
                try {
                  const captureResponse = await axios.post(
                    `http://localhost:8000/api/users/paypal/capture-order/`,

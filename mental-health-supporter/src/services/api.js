...
export async function registerDoctor(formData, isFormData = false) {
  ...
  const response = await fetch("http://localhost:8000/api/users/register/doctor/", options);
  if (!response.ok) {
    let errorData;
    try {
      errorData = await response.json();
...
export async function fetchDoctorById(doctorId) {
  ...
  const response = await fetch(`http://localhost:8000/api/users/doctors/${doctorId}/`, {
    method: 'GET',
    headers: {
...
const GENERAL_BASE_URL = 'http://localhost:8000/api';
